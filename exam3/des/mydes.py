from des import Des,form
from des import function as f
import numpy as np
from des.form import show
...
def CreateEncryptoFile(C,savepath):
    cipher_text=''
    with open(savepath+"\des_encrypto.txt", "w",encoding='UTF-8') as t:
        for cipher in C:
            cipher_text+=cipher
            t.write(cipher)
    return cipher_text
def CreateDecryptoFile(M,savepath):
    ...
    with open(savepath+"\des_decrypto.txt", "w",encoding='UTF-8') as t:
        t.write(msg)
    return msg
def DesEncrypto(filepath,savepath,key):
    M=PlaintextGroup(filepath)
    K=key
    # M=0x0123456789ABCDEF
    C=DesEncrypt(M,K)
    cipher_text=CreateEncryptoFile(C,savepath)
    print("des 加密结果:",cipher_text)
    return cipher_text
def DesDecrypto(filepath,savepath,key):
    with open(filepath, "r",encoding='UTF-8') as t:
        msg=t.read()
    msg_len=len(msg)
    data=[]
    ...
    # M=0x0123456789ABCDEF
    Mt=DesDecrypt(M,K)
    return CreateDecryptoFile(Mt,savepath)
